...
箭矢/弹药系统 (Ammo Standard)：
- 箭矢是**装备**而非消耗品：必须装备到 `Ammo` 槽位才能生效，严禁在 UI 中提供“直接使用”按钮。
- 堆叠属性：装备箭矢时，将背包中同 ID 的**全部数量**移入装备槽位；卸下时全额返还。
- 消耗机制：远程武器（如弓）攻击时必须检查并扣除 1 枚弹药，弹药耗尽必须停止攻击。

5. 视觉与 UI 准则 (Aesthetic Rules)
极简主义：保持控制台风格（Text-driven）。背包和列表应去图形化，**不显示物品图标**，仅显示必要的文字信息和状态（如 [插槽]、x数量）。

信息降噪：去除冗余的类型标签（如每个物品下的 "Usable" 文本），仅在必要时通过颜色或特殊符号区分。
